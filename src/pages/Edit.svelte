<script>
  ...
  let preview
  $: {
    if (chunk?.value && (id || showing_preview)) preview = chunkValueToHtml(chunk.value)
  }
  ...
  <div class="container" style:display={!_id ? "none" : "initial"}>
    {#if chunk}
      <div class="edit">
        ...
        />
        <button class="copy_id icon" on:click={copy_id}>
      ...
      <div class="preview" class:showing_preview>
        {@html preview}
        <button class="share icon" on:click={share}>
          <svg fill="currentColor" viewBox="0 0 16 16">
            <path
