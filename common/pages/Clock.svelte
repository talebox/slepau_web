<script>
  ...
  function update(settings, offset) {
    ...
    {
      // spehrical -> cartesian.
      const y = Math.sin(moon.altitude),
        x = Math.sin(moon.azimuth);
      // cartesian -> polar.
      moon_angle = Math.atan(y / x) + (moon.azimuth < 0 ? Math.PI : 0);
    }
    {
...
{#if !$is_phone$}
  ...
  >
    ...
    <p>
      Times<br />
      {#each Object.entries(times) as [k, t]}
        {k}: {`${t.getHours()}:${t.getMinutes().toString().padStart(2, "0")}`}
        <br />
      {/each}
    </p>
  </code>
...
<div class="stats-nice">
  ...
  {`${times.sunrise.getHours()}:${times.sunrise
    .getMinutes()
    .toString()
    .padStart(2, "0")}`}
  <svg fill="currentColor" viewBox="0 0 16 16">
  ...
  {`${times.sunset.getHours()}:${times.sunset
    .getMinutes()
    .toString()
    .padStart(2, "0")}`}
  <svg fill="currentColor" viewBox="0 0 16 16">
  ...
  {`${times.solarNoon.getHours()}:${times.solarNoon
    .getMinutes()
    .toString()
    .padStart(2, "0")}`}
  <svg fill="currentColor" viewBox="0 0 16 16">
  ...
  {`${times.nadir.getHours()}:${times.nadir
    .getMinutes()
    .toString()
    .padStart(2, "0")}`}
...
<style>
  ...
  .planet {
    ...
    width: max(min(90vw, 100vh), 480px);
    height: max(min(90vw, 100vh), 480px);
  }
  .back,
  .sunlight {
    cursor: pointer;
