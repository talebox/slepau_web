...
<Drawer>
  ...
  {#if user?.user !== "public"}
    <button class="menu" on:click={() => navigate(`${globalThis.PREFIX}/app/settings/`)}>
  ...
  <button class="menu" on:click={() => navigate(`${globalThis.PREFIX}/app/well/`)}>
    ...
    Well</button
  >
  <button class="menu" on:click={() => navigate(`${globalThis.PREFIX}/app/graph/`)}>
    <svg fill="currentColor" viewBox="0 0 16 16" class="icon">
      <path
        fill-rule="evenodd"
